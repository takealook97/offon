...
export default async function ApprovalsPage() {
  ...
  const [leavePending, leaveRecent, attPending] = await Promise.all([
  ...
  const recentItems = leaveRecent.map((l) => ({
    id: l.id,
    name: l.member.name,
    approverName: l.approver?.name ?? null,
    range: `${formatKST(l.startDate, 'MM-dd')} ~ ${formatKST(l.endDate, 'MM-dd')}`,
    days: Number(l.days),
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-muted-foreground">Recently handled (leave)</h2>
        <RecentLeaves items={recentItems} />
      </section>
    </div>
  );
}
